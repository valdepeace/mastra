...
A schedule runs an agent on a cron cadence. On each fire, Mastra sends a prompt either as a [signal](/docs/harness/signals) into a thread or as a threadless [`agent.generate()`](/reference/agents/generate) run. Use schedules for recurring work such as daily summaries and periodic checks, including scheduled nudges into a conversation.
